...
const Timeline: React.FC<TimelineProps> = ({ isAdmin, entries, onUpdate }) => {
  ...
  return (
    <section id="trajetoria" className="py-20 md:py-32 bg-white scroll-mt-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mb-20 md:mb-32 text-center">
          <div className="relative inline-block">
             <span className="absolute -top-10 left-1/2 -translate-x-1/2 text-8xl font-black text-slate-50 select-none z-0 tracking-widest">HISTORY</span>
             <div className="relative z-10">
                <span className="text-blue-900 font-bold uppercase tracking-[0.4em] text-[10px] block mb-2">Cronologia</span>
                <h2 className="text-3xl md:text-6xl font-bold text-slate-900 tracking-tight">Trajetória</h2>
                <div className="w-24 h-1.5 bg-gradient-to-r from-blue-900 via-green-600 to-blue-900 mx-auto mt-6 rounded-full"></div>
             </div>
          </div>
        </div>

        <div className="relative max-w-6xl mx-auto">
          <div className="absolute left-6 md:left-1/2 top-20 bottom-20 w-1 bg-slate-100 -translate-x-1/2 rounded-full overflow-hidden">
              ...
              return (
                <div key={idx} className="relative group">
                  <div className="absolute left-6 md:left-1/2 top-4 md:top-1/2 -translate-x-1/2 md:-translate-y-1/2 z-20">
                    <div className="w-8 h-8 md:w-16 md:h-16 bg-white rounded-lg md:rounded-xl rotate-45 border border-slate-200 shadow-xl flex items-center justify-center">
                       <div className="w-1 md:w-2 h-1 md:h-2 bg-blue-900 rounded-full"></div>
                    </div>
                  </div>

                  <div className={`flex flex-col md:flex-row items-stretch md:items-center w-full`}>
                    <div className={`hidden md:flex w-full md:w-1/2 items-center ${isEven ? 'md:order-2 justify-start pl-32 lg:pl-48' : 'md:order-1 justify-end pr-32 lg:pr-48'}`}>
                        <span 
                          className="text-sm font-bold text-slate-900 uppercase tracking-[0.2em] bg-white px-4 py-1 rounded-full shadow-sm border border-slate-100"
                    ...
                    <div className={`w-full md:w-1/2 pl-12 md:pl-0 ${isEven ? 'md:order-1 md:pr-32 lg:pr-48' : 'md:order-2 md:pl-32 lg:pl-48'}`}>
                      <div className="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
                        <div className={`relative z-10 flex flex-col ${isEven ? 'md:items-end' : 'items-start'}`}>
                          <div className={`flex items-center gap-4 mb-5 ${isEven ? 'md:flex-row-reverse' : 'flex-row'}`}>
                            <div className="flex-shrink-0 w-12 h-12 md:w-16 md:h-16 bg-slate-900 text-white rounded-xl flex items-center justify-center">
                              {getIcon(idx)}
                            </div>
                            <div className={isEven ? 'md:text-right' : 'text-left'}>
                              <h3 
                                className="text-lg md:text-3xl font-bold text-slate-900 leading-tight mb-1"
                                contentEditable={isAdmin}
                              ...
                              <p 
                                className="text-green-700 font-black text-[10px] md:text-sm uppercase tracking-[0.15em]"
                                contentEditable={isAdmin}
                          ...
                          <p 
                            className={`text-slate-600 text-sm md:text-lg leading-relaxed mb-6 font-light ${isEven ? 'md:text-right' : 'text-left'}`}
                            contentEditable={isAdmin}
                          ...
                          {entry.highlights && (
                            <div className={`flex flex-wrap gap-2 ${isEven ? 'md:justify-end' : 'justify-start'}`}>
                              {entry.highlights.map((item, hIdx) => (
                                <span 
                                  key={hIdx} 
                                  className="text-[9px] md:text-[11px] font-bold uppercase tracking-wider bg-slate-50 text-slate-600 border border-slate-200/60 px-3 py-1.5 rounded-xl transition-all"
                                  contentEditable={isAdmin}
                                  onBlur={(e) => {
